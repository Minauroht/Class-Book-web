...
Math.minmax = (value, limit) => {
  ...
  function resetGame() {
    ...
    if (hardMode) {
      noteElement.innerHTML = ` 조이스틱을 움직여 시작하세요
          <p>공들을 가운데로 움직여 클리어하세요. E를 누르면 이지모드로 돌아갈 수 있습니다.</p>`;
    } else {
      noteElement.innerHTML = ` 조이스틱을 움직여 시작하세요
          <p>공들을 가운데로 움직여 클리어하세요. H를 누르면 하드모드를 즐길 수 있습니다.</p>`;
    }
  ...
  function main(timestamp) {
    ...
    try {
      ...
      if (
        ...
        noteElement.innerHTML = `축하해요 클리어 성공!
          ${!hardMode ? "<p>H를 눌러 하드모드에 도전해보세요!</p>" : ""}
          <p>
            도움을 주신 분
            <a href="https://www.youtube.com/watch?v=bTk6dcAckuI" , target="_blank"
              >@HunorBorbely</a
            >
          </p>`;
        noteElement.style.opacity = 1;
      ...
      if (error.message == "The ball fell into a hole") {
        noteElement.innerHTML = `공이 블랙홀에 빠져버렸어요
          <p>
            이지모드로 돌아가려면 E를 누르세요
          </p>`;
        noteElement.style.opacity = 1;
